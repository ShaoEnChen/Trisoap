...
if($EMAIL != null && $CUSIDT == 'A'){
    echo "待執行訂單：<br>";
    $queryORDMAS = "SELECT * FROM ORDMAS WHERE ACTCODE=1 AND ORDSTAT='E' AND PAYSTAT=1";
    $result = mysql_query($queryORDMAS);
    while($row = mysql_fetch_array($result)){
        $ORDNO = $row['ORDNO'];
        echo "訂單編號:";
        echo "<form name=\"form\" method=\"post\" action=\"View_ORDITEM_E.php\">";
